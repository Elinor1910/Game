    ...
    def turns(self):
        ...
        for player in self.players:
            if player == self.players[self.current_turn]:
                ...
                self.sendallclients(player)

    def update(self, msg):
        counter = 0
        for player in self.players:
            if self.decode_name_from_clients(player) == self.decode_name_from_clients(msg):
                self.players[counter] = msg
                counter += 1
            counter += 1
        self.current_turn += 1
        self.turns()

    def decode_name_from_clients(self, msg):
        x = msg.split()
        name = x[1]
        return name
